...
export default function Header() {
  ...
  const navFixedClass = `${styles.navFixed} ${isScrolled ? styles.scrolled : ''}`;

  return (
    <header className={headerClass}>
      <div className={styles.left} onClick={()=> router.push('/')}>
        <svg className={styles.logo} viewBox="0 0 33 32" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className={navFixedClass}>
      <nav className={styles.nav}>
        <ul className={styles.navList}>
          <li
            className={`${styles.navItem} ${styles.homeButton}`}
            onClick={() => router.push('/')}
            title="Home"
          >
